...
class TestMetrics():
    ...
    @staticmethod
    def test_predictions_subtoken(predictions, targets, batch_tokens, vocab, score_map={'exact': 2, 'partial': 1, False: 0}):
        prediction_locs = TestMetrics.get_locs(predictions)
        target_locs = TestMetrics.get_locs(targets)
        ...
        for p_loc, t_loc in it.product(prediction_locs, target_locs):
            score = TestMetrics.score_overlap(p_loc,t_loc)
    ...
    @staticmethod
    def test_predictions_token(predictions, targets, batch_tokens, vocab, score_map={'exact': 2, 'partial': 1, False: 0}):
        subtokens = [vocab[t] for t in batch_tokens]
        subtoken_map = TestMetrics.map_subtokens(subtokens)
        targets = TestMetrics.subtoken2token_labels(targets, subtoken_map)
        predictions = TestMetrics.subtoken2token_labels(predictions, subtoken_map)

        return TestMetrics.test_predictions_subtoken(predictions, targets, batch_tokens, vocab, score_map=score_map)
    ...
    def sequence_overlap_twoway(s1,s2):
        if TestMetrics.sequence_overlap(s1,s2): 
            return set(s1) & set(s2)
        if TestMetrics.sequence_overlap(s2,s1): 
            return set(s1) & set(s2)
        return False
    ...
    @staticmethod
    def test_predictions_nonpositional(predictions, targets, batch_tokens, vocab, threshold=0.5):
        prediction_tokens = TestMetrics.get_labeled_tokens(predictions, batch_tokens)
        target_tokens = TestMetrics.get_labeled_tokens(targets, batch_tokens)
        ...
        for p_tokens,t_tokens in it.product(prediction_tokens,target_tokens):
            overlap = TestMetrics.sequence_overlap_twoway(p_tokens,t_tokens)
            if overlap:
    ...
    @staticmethod
    def test_all(outputs, vocab, f, f_kwargs={}, token=False):
        exacts = 0
        partials = 0
        misses = 0
        spurious = 0

        for batch,predictions in outputs:
            targets = batch['labels'].squeeze(dim=0).tolist()
            tokens = batch['tokens'].squeeze(dim=0).tolist()
            
            for i in range(len(predictions)):
                batch_exacts, batch_partials, batch_misses, batch_spurious = f(predictions[i], targets[i], tokens[i], vocab, **f_kwargs)
        ...
        exact = TestMetrics.get_metrics(exacts,misses,spurious)  # exact
        partial = TestMetrics.get_metrics(exacts+partials,misses,spurious)  # partial
